...
def extract_data_api():
    global revenue_data, net_income_data, FCF_data, profit_margin_data, common_stock_shares_outstanding_data, \
        assets_vs_liabilities_growth, stock_price_growth, data1, data2, data3, data4
    ...
    url4 = f"https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY_ADJUSTED&symbol={symbol}&apikey={api_key}"
    response1, response2, response3, response4 = requests.get(url1), requests.get(url2), requests.get(
        url3), requests.get(url4)
    revenue_data, net_income_data, FCF_data, profit_margin_data, common_stock_shares_outstanding_data, \
        assets_vs_liabilities_growth, stock_price_growth = [], [], [], [], [], [], []
    if all(response.status_code == 200 for response in [response1, response2, response3, response4]):
        data1, data2, data3, data4 = response1.json(), response2.json(), response3.json(), response4.json()
        if "annualReports" in data1 and "annualReports" in data2 \
                and "annualReports" in data3 and "Monthly Adjusted Time Series" in data4:
            extract_data1()
            extract_data2()
            extract_data3()
            extract_data4()
...
def extract_data4():
    stock_price_2023 = float(data4["Monthly Adjusted Time Series"]["2023-09-29"]["5. adjusted close"])
    stock_price_2020 = float(data4["Monthly Adjusted Time Series"]["2021-12-31"]["5. adjusted close"])
    stock_price_growth.append(round((stock_price_2023 - stock_price_2020) / stock_price_2020 * 100, 2))
...
def write_data_to_csv(csvw):
    csvw.writerow(reversed(revenue_data + net_income_data + profit_margin_data +
                           FCF_data + common_stock_shares_outstanding_data +
                           four_years_growth + assets_vs_liabilities_growth + stock_price_growth))


with open("stocks_fundamentals2.csv", "a", newline="") as csvfile:
